 ...
 * 4. When the user types in a field, updates that field in the form data 
 * (and other fields can just reuse the previous data)
 * 5. When the user submits the form sends the updated data to the backend
 */

/** Add expense window code logic:
 * Reason: Reuse what we had (Edit Expense Window) as these two windows have similar features
 * 1. Detect whether the user is editing or adding
...
/** Current expense object we want to edit, 
...
function ExpenseEditWindow({ expense, tripId, expenses, onClose, onSave }) {
  ...
  // Initialise form with empty strings
  ...
  // Load all categories from the server once component mounts
  useEffect(() => {
  ...
  const handleChange = (e) => {
    ...
    // Fetch previous state, only change the updated one
